...
bool WindowManager::init(int width, int height, const std::string& title) {
    ...
    font_cfg.Flags |= ImFontAtlasFlags_None; // Standard flags
    // Note: We'd use ImFontConfig to pass NoLoadError if the API supported it easily, 
    // but since we want to avoid the assert, we'll check file existence manually or
    // just use a more cautious approach.
    
    for (const char* font_path : linux_fonts) {
        FILE* f = fopen(font_path, "rb");
        if (f) {
            fclose(f);
            if (io.Fonts->AddFontFromFileTTF(font_path, 16.0f)) {
                std::cout << "Loaded Linux font: " << font_path << std::endl;
                font_loaded = true;
                break;
            }
        }
